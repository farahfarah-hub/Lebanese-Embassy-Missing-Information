        ...
        if (verdict === 'correct') {
            ownReviewRadios
                ...
                .forEach(r => {
                    ...
                    // Flash the pill ONLY when (1) the reviewer just clicked
                    // the verdict (not on initial load) and (2) the radio
                    // actually transitioned from unchecked → checked. This
                    // gives unambiguous "yes the auto-tick fired" feedback.
                    if (userInitiated && !wasChecked) {
                        flashPill(r.closest('.radio-pill'));
                    }
                });
        } else {
            ownReviewRadios
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        document.body.addEventListener("change", (e) => {
            ...
            if (t && t.dataset && t.dataset.verdict === "true") {
                ...
                if (sec) applyVerdictStateToSection(sec, /*userInitiated*/true);
            }
            scheduleSave();
            updateProgress();
        });
